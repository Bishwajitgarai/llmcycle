...
# ─── OpenAI-Compatible Ingress Proxy Server ───────────────────────────────────
@app.post("/v1/chat/completions")
async def openai_proxy_chat_completions(req: Dict[str, Any]):
    """OpenAI-compatible proxy chat completion endpoint."""
    model = req.get("model")
    if not model:
    ...
    # Extract extra generation parameters
    gen_kwargs = {}
    ...
    if stream:
        async def event_generator():
            try:
                ...
                # Final chunk to signal stream end
                final_chunk = {
                    "id": chunk_id,
                    "object": "chat.completion.chunk",
                    "created": created_ts,
...
async def list_models_openai():
    """Return list of models across all active providers in OpenAI format."""
    all_models = []
